...
<body class="padTop53" style="background:url(themes/images/bg.png)">
	...
	<div class="well" style="fixed:center;">
		...
		<p style="margin-top:10px;">
			...
			include ('koneksi.php');
$kode = $_GET['ip'];
$tampil = "select * from produk where idproduk = '$kode' order by idproduk DESC ";
$query = mysql_query($tampil,$koneksi);
$row = mysql_fetch_array($query);
if($_POST["btn"]){
	if(isset($_POST['edit'])){
			include ('koneksi.php');
$perintah = "update produk set namaproduk='".$_POST["np"]."', foto='".$_POST["ip"].$_FILES["ft"]['name']."' where idproduk='".$_POST["ip"]."'";
$ubah = mysql_query($perintah,$koneksi);
if($ubah){
unlink("fotoproduk/".$row['foto']);
move_uploaded_file( $_FILES["ft"]["tmp_name"],"fotoproduk/".$_POST["ip"].$_FILES["ft"]['name']);
echo("<script>
		

swal('Selamat!', 'Data Produk sukses diubah', 'success');
				
		</script>");
			echo "<center><div class='alert alert-success alert-dismissable'>
                  <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
					<b>Berhasil mengubah data produk!!</b>
			</div><center>
...
echo "<center><div class='alert alert-warning alert-dismissable'>
                  <button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
					<b>Gagal mengubah data produk!!</b>
			</div><center>";
}
...
include ('koneksi.php');
$perintah = "update produk set namaproduk='".$_POST["np"]."' where idproduk='".$_POST["ip"]."'";
$ubah = mysql_query($perintah,$koneksi);
if($ubah){
echo("<script>
...
?>
			...
			<div class="form-group">
							...
							<div class="col-lg-4">
								<input type="text" name="ip" autofocus="" required="" class="form-control" value="<?php echo $row['idproduk']; ?>" readonly >
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-lg-4">Nama Produk</label>
							<div class="col-lg-4">
								<input type="text" name="np" autofocus="" required="" class="form-control" value="<?php echo $row['namaproduk']; ?>">
							</div>
						</div>
												
						<div class="form-group">
							<label class="control-label col-lg-4">Ubah foto produk?</label>
							<div class="col-lg-2">
						...
						<div class="form-actions no-margin-bottom" style="text-align:center;">
							<input type="submit" name="btn" value="Simpan" class="btn btn-primary"> <a href="produk.php" class="btn btn-warning">Kembali</a>
						</div>
