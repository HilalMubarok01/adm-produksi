...
if(isset($_POST["cari"])){
$nama = $_POST["cari"];
		$tampil = "select * from tborder where idorder LIKE '%$nama%' or idadmin LIKE '%$nama%' or namaadmin LIKE '%$nama%' or idproduk LIKE '%$nama%' or namaproduk LIKE '%$nama%' or tglorder LIKE '%$nama%' or tgltarget LIKE '%$nama%' order by idorder DESC LIMIT $hal, $data";
$query = mysql_query($tampil,$koneksi);
}elseif(isset($_GET["cari"])){
$nama = $_GET["cari"];
		$tampil = "select * from tborder where status LIKE '%$nama%' order by idorder DESC LIMIT $hal, $data";
$query = mysql_query($tampil,$koneksi);
}else{
$tampil = "select * from tborder order by idorder DESC LIMIT $hal, $data";
$query = mysql_query($tampil,$koneksi);
}
?>
							<tbody>
				...
				$nomor++;
					 ?>
					  ...
					  <td><?php echo $row['namaadmin']; ?></td>
                        ...
                        <td><?php echo $row['tgltarget']; ?></td>
                        <td><?php echo $row['ket']; ?></td>
                        <td>
                        <?php if ($row['foto'] != ""){
                        echo '<a href="fotoorder/'.$row['foto'].'" target="_blank"><img width="50px" height="50px" src="fotoorder/'.$row['foto'].'"></a>';}else{echo '<a href="fotoorder/default.png" target="_blank"><img width="50px" height="50px" src="fotoorder/default.png"></a>';}
						 ?></td>
                        <td>
                        <?php
						if($row['status']=="Menunggu konfirmasi produksi"){
						 echo $row['status'];
						?>
						<?php if($_SESSION["hakakses"]=="Super Admin"){ ?>
						<br>
                          <a href="status.php?io=<?php echo $row['idorder']; ?>" class="btn btn-xs btn-info tipsy-kiri-atas">Konfirmasi produksi</a>
                         <?php } ?>
						<?php
						}elseif($row['status']=="Menunggu konfirmasi bahan"){
						 echo $row['status'];
						?>
						<?php if($_SESSION["hakakses"]=="Super Admin"){ ?>
						<br>
                          <a href="status.php?io=<?php echo $row['idorder']; ?>" class="btn btn-xs btn-info tipsy-kiri-atas">Konfirmasi bahan</a>
                          <?php } ?>
						<?php
						}elseif($row['status']=="Belum konfirmasi bahan"){
						 echo $row['status'];
						
						  ?>
						  <?php if($_SESSION["hakakses"]=="Super Admin"){ ?>
                          <br>
                          <a href="status.php?io=<?php echo $row['idorder']; ?>" class="btn btn-xs btn-info tipsy-kiri-atas">Konfirmasi & siapkan bahan</a>
                          <?php } ?>
                                <?php }else{ echo $row['status'];} ?>
                        </td>
                        <?php if($_SESSION["hakakses"]=="Super Admin"){ ?>
						 <td><a href="rincianorder.php?io=<?php echo $row['idorder']; ?>" class="btn btn-xs btn-metis-5" title='Rincian Order'>Lihat rincian</a></td>
                        
                      	<td>
						<div class='btn-group'>
						  <a href="hapusorder.php?io=<?php echo $row['idorder']; ?>&ft=<?php echo $row['foto']; ?>" class="btn btn-xs btn-danger tipsy-kiri-atas" title="Hapus Data Ini" onClick="return confirm('ANDA YAKIN AKAN MENGHAPUS DATA PENTING INI ... ?')">Hapus</a> 
						  </div>
						</td>
					<?php } ?>
					</tr>
				...
				<tr>
						...
						for($h = 1; $h <= $max; $h++){
							$list[$h] = $data*$h-$data;
							echo "<ul class='pagination pagination-sm'><li><a href='order.php?hal=$list[$h]'>$h</a></li></ul>";
						}
						?></td>
			</tbody></table>
		</div>
