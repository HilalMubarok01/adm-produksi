...
error_reporting(0);
session_start();
if($_SESSION["emailadmin"]!="" AND $_SESSION["passwordadmin"]!="" AND $_SESSION["hakakses"]=="Super Admin"){
?>
<!DOCTYPE html>
...
<a href="inputadmin.php" class="btn btn-primary btn-rect">Tambah Data</a><br><br>
	<label>Cari</label><br>
    <input type="text" name="cari" autofocus="" required="" style="width:200px;"  >
	<button type="submit" id="btncari" name"btncari"  class="btn btn-primary btn-rect">Cari Data</button><br>
</form>	<br>					
<div class="panel panel-default">
	<div class="panel-heading">
		Tabel Data Admin	</div>
...
if(isset($_POST["cari"])){
$nama = $_POST["cari"];
		$tampil = "select * from admin where idadmin LIKE '%$nama%' or namaadmin LIKE '%$nama%' or email LIKE '%$nama%' order by idadmin DESC LIMIT $hal, $data";
$query = mysql_query($tampil,$koneksi);
}else{
$tampil = "select * from admin order by idadmin DESC LIMIT $hal, $data";
$query = mysql_query($tampil,$koneksi);
}
?>
							<tbody>
				...
				while ($row = mysql_fetch_array($query)) {
					 ...
					 ?>
						...
						<td>
                        <?php if ($row['foto'] != ""){
                        echo '<a href="fotoadmin/'.$row['foto'].'" target="_blank"><img width="50px" height="50px" src="fotoadmin/'.$row['foto'].'"></a>';}else{echo '<a href="fotoadmin/default.png" target="_blank"><img width="50px" height="50px" src="fotoadmin/default.png"></a>';}
						 ?></td>
						<td>
						<div class='btn-group'>
						  <a href="hapusadmin.php?ia=<?php echo $row['idadmin']; ?>&ft=<?php echo $row['foto']; ?>" class="btn btn-xs btn-danger tipsy-kiri-atas" title="Hapus Data Ini" onClick="return confirm('ANDA YAKIN AKAN MENGHAPUS DATA PENTING INI ... ?')">Hapus</a> 
						  <a href="editadmin.php?ia=<?php echo $row['idadmin']; ?>" class="btn btn-xs btn-info tipsy-kiri-atas" title='Edit Data ini'>Edit Data</a>
						  </div>
						</td>
					</tr>
					<?php } ?>
				...
				<tr>
						...
						for($h = 1; $h <= $max; $h++){
							$list[$h] = $data*$h-$data;
							echo "<ul class='pagination pagination-sm'><li><a href='admin.php?hal=$list[$h]'>$h</a></li></ul>";
						}
						?></td>
			</tbody></table>
		</div>
